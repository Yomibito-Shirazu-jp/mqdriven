...
export const extractSpecsFromContent = async (text: string, imageData?: { data: string, mimeType: string }): Promise<{ specs: PrintSpecs, sources: GroundingSource[] }> => {
  ...
  ` }];
    ...
    return {
        specs: JSON.parse(result.response.text()) as PrintSpecs,
        sources: []
    };
};
            ...
            responseSchema: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        pdfReference: { type: Type.STRING, description: "PDF内の具体的な根拠箇所（ページ、座標、要素名など）" },
...
export const calculateDeliveryImpact = async (destination: string): Promise<GroundingSource[]> => {
    ...
    const result = await model.generateContent({
        ...
        tools: [
            { googleSearch: {} } as any // Using Search for logistics logic instead of specialized Maps tool if not available in SDK
        ]
    });

    return result.response.candidates?.[0]?.groundingMetadata?.groundingChunks
        ?.filter((c: any) => c.web)
