    ...
    <>
      ...
      >
        ...
        <div className="sm:hidden flex items-center justify-between px-3 py-4 border-b border-slate-200 dark:border-slate-800">
          <h1 className="text-lg font-bold text-slate-800 dark:text-slate-100">文唱堂印刷 業務管理</h1>
          <button
            type="button"
            onClick={() => {
        ...
        <div className={`mt-2 flex flex-wrap gap-1 text-[10px] text-slate-500 dark:text-slate-500 ${isCollapsed ? 'justify-center' : ''}`}>
          {isCollapsed && (
            <button
              type="button"
              onClick={toggleSidebar}
          ...
          <a href="https://erp.b-p.co.jp" target="_blank" rel="noopener noreferrer" className={`px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors ${isCollapsed ? 'block w-6 h-6 text-center leading-6' : ''}`} title="業務">業</a>
          <a href="https://mq.b-p.co.jp" target="_blank" rel="noopener noreferrer" className={`px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors ${isCollapsed ? 'block w-6 h-6 text-center leading-6' : ''}`} title="MQ">MQ</a>
          <a href="https://dtp.b-p.co.jp" target="_blank" rel="noopener noreferrer" className={`px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors ${isCollapsed ? 'block w-6 h-6 text-center leading-6' : ''}`} title="DTP">D</a>
          <a href="https://co2.b-p.co.jp/" target="_blank" rel="noopener noreferrer" className={`px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 transition-colors ${isCollapsed ? 'block w-6 h-6 text-center leading-6' : ''}`} title="エコ">E</a>
        </div>
            ...
            {visibleCategories.map(category => {
              ...
              return (
                <React.Fragment key={category.id}>
                  <li className={`mt-6 px-3 text-[11px] font-bold text-slate-400 dark:text-slate-500 tracking-wider mb-1 ${shouldShowDesktop ? '' : 'sr-only'} ${!shouldShowMobile ? 'hidden sm:block' : ''}`}>
                    {!isCollapsed && (
                      <button
                        type="button"
                        onClick={() => toggleCategory(category.id)}
                        className="flex items-center w-full hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                        ...
                        >
                          ...
                          {item.children && !isCollapsed && (
                            <button
                              ...
                              className="ml-auto p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700"
        ...
        <div className="mt-auto pt-4 border-t border-slate-200 dark:border-slate-800 space-y-4">
          {supabaseUserEmail && (
            <div className="px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
              ...
              <p className="text-[13px] font-semibold text-slate-700 dark:text-slate-200 break-all">{supabaseUserEmail}</p>
            </div>
          )}
          {onSignOut && (
            <button
              type="button"
              onClick={onSignOut}
              className="w-full px-3 py-2 text-[13px] font-semibold text-center text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100 rounded-md transition-colors"
            >
              ログアウト
            </button>
          )}
